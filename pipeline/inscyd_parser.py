"""Standalone parser for INSCYD PDF reports."""
...
class ParsedInscydReport:
    ...
    projected_bsa_m2: float | None = None
    body_fat_percent: float | None = None
    body_fat_kg: float | None = None
    fat_free_percent: float | None = None
    fat_free_kg: float | None = None
    vo2max_abs_ml_min: float | None = None
    vo2max_rel_ml_kg_min: float | None = None
    vlamax_mmol_l_s: float | None = None
    mfo_abs_kcal_h: float | None = None
    mfo_rel_kcal_h_kg: float | None = None
    fatmax_watt: float | None = None
    carbmax_abs_watt: float | None = None
    carbmax_rel_w_kg: float | None = None
    at_abs_watt: float | None = None
    at_rel_w_kg: float | None = None
    at_pct_vo2max: float | None = None
    glycogen_abs_g: float | None = None
    glycogen_rel_g_kg: float | None = None
    hr_max_bpm: int | None = None
    pwc150_watt: float | None = None
...
class InscydParser:
    """Extract structured metrics from an INSCYD PDF export."""

    def parse_file(self, file_path: str | Path) -> ParsedInscydReport:
        ...
        report.body_height_cm = self._extract_float(compact, r"Body Height\s+([0-9.]+)\s*cm")
        report.body_mass_index = self._extract_float(compact, r"Body Mass Index\s+([0-9.]+)\s*kg/m2")
        report.projected_bsa_m2 = self._extract_float(compact, r"Projected BSA\s+([0-9.]+)\s*m2")
        report.body_fat_percent = self._extract_float(compact, r"Body Fat\s+([0-9.]+)\s*% of body mass /")
        report.body_fat_kg = self._extract_float(compact, r"Body Fat\s+[0-9.]+\s*% of body mass /\s*([0-9.]+)\s*kg")
        report.fat_free_percent = self._extract_float(compact, r"Fat Free\s+([0-9.]+)\s*% of body mass /")
        report.fat_free_kg = self._extract_float(compact, r"Fat Free\s+[0-9.]+\s*% of body mass /\s*([0-9.]+)\s*kg")
        ...
        report.vo2max_abs_ml_min = self._extract_float(benchmark_block, r"Absolute:\s*([0-9.]+)\s*ml/min")
        report.vo2max_rel_ml_kg_min = self._extract_float(benchmark_block, r"Relative:\s*([0-9.]+)\s*ml/min/kg")
        report.vlamax_mmol_l_s = self._extract_float(benchmark_block, r"VLamax:\s*([0-9.]+)\s*mmol/l/s")
        report.mfo_abs_kcal_h = self._extract_float(benchmark_block, r"MFO Absolute:\s*([0-9.]+)\s*kcal/h")
        report.mfo_rel_kcal_h_kg = self._extract_float(benchmark_block, r"MFO Relative:\s*([0-9.]+)\s*kcal/h/kg")
        report.fatmax_watt = self._extract_float(benchmark_block, r"Fatmax:\s*([0-9.]+)\s*Watt")
        report.at_pct_vo2max = self._extract_float(benchmark_block, r"%VO2max:\s*([0-9.]+)\s*%")
        report.glycogen_abs_g = self._extract_float(benchmark_block, r"Absolute:\s*([0-9.]+)\s*g")
        report.glycogen_rel_g_kg = self._extract_float(benchmark_block, r"Relative:\s*([0-9.]+)\s*g/kg")
        report.hr_max_bpm = self._extract_int(benchmark_block, r"Maximum:\s*([0-9.]+)\s*bpm")
        report.pwc150_watt = self._extract_float(benchmark_block, r"PWC150:\s*([0-9.]+)\s*Watt")

        absolute_watts = self._extract_all_floats(benchmark_block, r"Absolute:\s*([0-9.]+)\s*Watt")
        relative_watts = self._extract_all_floats(benchmark_block, r"Relative:\s*([0-9.]+)\s*Watt/kg")
        if absolute_watts:
        ...
        tables_block = self._extract_section(compact, "Test Data - tables", "Training Zones")
        report.raw_sections["test_data_tables"] = tables_block
        report.test_data_rows = self._parse_test_data_rows(tables_block)
        report.weighted_regression = self._parse_weighted_regression(tables_block)
    ...
    def _extract_section(self, text: str, start_marker: str, end_marker: str | None) -> str:
        if end_marker:
            ...
            match = re.search(rf"{re.escape(start_marker)}\s+(.*)$", text, re.IGNORECASE)
            ...
            rows.append(
                {
                    ...
                    "additional_value": row_match.group(4).strip() if row_match.group(4) else None,
                }
            )
        return rows
