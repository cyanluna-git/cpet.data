"""
Belgium Lactate Test — Static HTML Report Generator
Phase 5: Render a standalone Korean analysis report from SQLite outputs.
"""
...
DB_PATH = Path(__file__).resolve().parent / "belgium_lactate.db"
DOC_DIR = DB_PATH.parent.parent
...
def build_report_context(db_path: Path = DB_PATH) -> dict[str, Any]:
...
<body>
  <main class="page">
    ...
    <p class="footer-note">Generated at {html_text(context["meta"]["generated_at"])} · Source DB: {html_text(str(DB_PATH.name))}</p>
...
def write_report(db_path: Path = DB_PATH, output_path: Path = REPORT_PATH) -> Path:
    """Build context, render HTML, and write the final report file."""
...
def main() -> None:
    ...
    print(f"report.html written to {report_path}")
