...
- `높음`
- 현재 ZeLIA의 수집 구조와 가장 전통적으로 잘 맞는 주제라서, 논문으로 정리하기 가장 수월한 축이다.

### 근거력 확보를 위한 데이터 규모

- 최소: `lactate 포함 CPET 40~60건`
- 권장: `50명 이상`, 그중 `10~20명`은 반복 측정 포함
- 이유: 개인 간 일치도와 subgroup 차이를 같이 보려면 단순 사례 수준을 넘는 표본이 필요하다.

### 데이터 확보 방법

1. 기존 `lactate 포함 CPET`를 전수 정리한다.
...
- `매우 높음`
- 이미 계산하고 있는 `FatMax`, `crossover`, `RQ 1.0 fuel split`, `feature_sets`와 직접 이어져서 현재 시스템과 가장 잘 맞는다.

### 근거력 확보를 위한 데이터 규모

- 최소: `CPET 60~80건`
- 권장: `80~120건`, 그중 `15~20명`은 반복 측정 포함
- 이유: 지표를 “개발”만 하지 말고, 최소한 내부 검증까지 보여야 논문 근거력이 생긴다.

### 데이터 확보 방법

1. 모든 CPET에서 `FatMax`, `crossover`, `RQ 1.0 crossing`, `fat/CHO contribution`를 일관되게 산출한다.
...
- `중간~높음`
- 출발 인프라는 이미 갖춰졌지만, 결국 반복 측정 데이터 밀도가 충분해야 주제가 강해진다.

### 근거력 확보를 위한 데이터 규모

- 최소: `20명 이상이 2회 측정`
- 권장: `20~30명 이상이 3회 이상 측정`, 총 `60~90 session`
- 이유: “궤적”을 말하려면 3시점 이상이 늘어나야 하고, 사람 수보다 반복밀도가 더 중요하다.
...
- `중간`
- 가장 AI다운 주제지만, 실제 논문 방어 난이도는 가장 높다. 지금 단계에서는 “대체 모델”보다 “가능성 평가”가 맞다.

### 근거력 확보를 위한 데이터 규모

- 최소: `lactate 포함 CPET 80~120건`
- 권장: `100건 이상`, `30명 이상`, stage 기준으로는 `2,000~3,000 paired sample`
- 이유: subject-wise split을 걸면 usable sample이 빠르게 줄어서, 생각보다 훨씬 많은 데이터가 필요하다.

### 데이터 확보 방법

1. lactate 포함 프로토콜을 우선적으로 표준화한다.
...
- `중간~높음`
- 데이터가 조금만 더 쌓이면 바로 시도 가능한 주제이고, 개인화 리포트 전략과도 잘 연결된다.

### 근거력 확보를 위한 데이터 규모

- 최소: `unique subject 60~80명`
- 권장: `100명 이상`, 가능하면 일부 반복 측정 포함
- 이유: 군집화는 반복 session 수보다도 “서로 다른 사람의 다양성”이 더 중요하다.
