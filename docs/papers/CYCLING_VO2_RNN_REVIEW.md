...
이 논문은 “가슴에 마스크를 쓰고 VO2를 직접 재지 않아도, 다른 센서 신호만으로 VO2를 어느 정도 추정할 수 있을까?”를 실험한 것이다.
...
이 신호들만 보고 “지금 몸이 산소를 이 정도 쓸 것 같다”를 맞출 수 있지 않을까?

논문은 여기에 대해:

- “가능성은 있다”
- “하지만 아직 작은 파일럿 연구다”
- “실측 VO2를 바로 대체한다고 보기엔 이르다”

라고 말하는 것으로 보면 된다.

## 0-1. 용어 풀이

- `VO2`: 산소 섭취량. 운동 중 몸이 얼마나 산소를 쓰는지
- `HR`: 심박수
- `power`: 자전거에서 내는 힘
- `cadence`: 페달 회전수
- `RF`: 호흡수
- `RNN`: 시간 흐름이 있는 데이터를 다루는 신경망. 이전 시점 정보를 다음 예측에 반영하는 모델
...
이 논문은 “속도계 없이 엔진 소리와 RPM만 듣고 지금 차가 얼마나 빨리 달리고 있을지 추정해보자”에 조금 비슷하다.

- 완전히 불가능한 일은 아니다.
- 잘 맞을 때도 있다.
- 하지만 실제 속도계를 떼어내고 그 추정치만 믿기엔 아직 위험하다.

ZeLIA에서도 마찬가지다.

- measured CPET는 실제 속도계 같은 것
- surrogate VO2 모델은 보조 추정기 같은 것

따라서 당장은 “보조 설명”으로는 좋지만, “실측 대체”로 말하면 안 된다.
...
## 4-1. 왜 초심자에게도 이게 중요하냐

이 논문이 중요한 이유는, 나중에 ZeLIA가 이런 질문에 답할 가능성을 열어주기 때문이다.

- 실험실 밖에서도 산소 사용 패턴을 대략 볼 수 있을까?
- CPET를 한 번 하고 나면 이후 라이딩 데이터만으로 변화를 추정할 수 있을까?
- 심박수와 파워가 비슷한데 산소 요구량 패턴은 달라지는지 볼 수 있을까?

하지만 동시에 한계도 분명하다.

- 표본이 너무 적다
- 입력 신호가 현실과 안 맞을 수 있다
- 사람마다 따로 보정해야 할 가능성이 크다

즉 “가능성은 보이지만, 바로 제품 버튼으로 만들 단계는 아니다”가 핵심이다.
...
- `MetFlex` 논문은 “현재 기능을 어떻게 안전하게 말할 것인가”에 관한 노트라면,
- 이 `Cycling VO2` 논문은 “나중에 어떤 연구 모델을 붙여볼 수 있는가”에 관한 노트다.

즉 지금 당장 제품 핵심은 아니지만, 미래 연구 방향으로는 충분히 참고할 가치가 있다.
